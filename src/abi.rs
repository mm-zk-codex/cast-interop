...
mod tests {
    use super::*;
    use alloy_primitives::{keccak256, B256, Bytes, U256 as AlloyU256};
    use alloy_sol_types::SolValue;

    #[test]
    fn error_selector_map_covers_all_expected_errors() {
    ...
    #[test]
    fn error_selector_map_bundle_already_processed_selector() {
        // Selector from code comment: 0x5bba5111
        let map = error_selector_map();
        assert_eq!(map.get("5bba5111"), Some(&"BundleAlreadyProcessed"));
    }

    #[test]
    fn error_selector_map_message_not_included_selector() {
        // Selector from code comment: 0x32c2e156
        let map = error_selector_map();
        assert_eq!(map.get("32c2e156"), Some(&"MessageNotIncluded"));
    }

    #[test]
    fn event_topics_are_nonzero() {
    ...
    #[test]
    fn event_topics_are_all_distinct() {
        let topics = [
        ...
        for i in 0..topics.len() {
            for j in (i + 1)..topics.len() {
                assert_ne!(topics[i], topics[j], "topics[{i}] == topics[{j}]");
            }
        }
    }
    ...
    fn decode_bytes32_roundtrip() {
        let original = B256::from([0xABu8; 32]);
    ...
    #[test]
    fn decode_bytes32_empty_data_errors() {
        assert!(decode_bytes32(Bytes::new()).is_err());
    }

    #[test]
    fn decode_bundle_status_all_enum_values() {
        for status in [0u8, 1, 2, 3] {
            let encoded = Bytes::from(AlloyU256::from(status).abi_encode());
            assert_eq!(decode_bundle_status(encoded).unwrap(), status);
        }
    }

    #[test]
    fn decode_call_status_values() {
        for status in [0u8, 1, 2] {
            let encoded = Bytes::from(AlloyU256::from(status).abi_encode());
            assert_eq!(decode_call_status(encoded).unwrap(), status);
        }
    }

    #[test]
    fn decode_u8_reads_first_byte() {
        assert_eq!(decode_u8(Bytes::from(vec![42u8, 0, 0, 0])).unwrap(), 42);
    }

    #[test]
    fn decode_u8_empty_errors() {
        assert!(decode_u8(Bytes::new()).is_err());
    }

    #[test]
    fn encode_bundle_status_call_correct_length_and_selector() {
        let hash = B256::from([1u8; 32]);
        let encoded = encode_bundle_status_call(hash);
        // 4-byte selector + 32-byte hash
        assert_eq!(encoded.len(), 36);
        let expected_selector = &keccak256("bundleStatus(bytes32)")[..4];
        assert_eq!(&encoded[..4], expected_selector);
    }

    #[test]
    fn encode_interop_roots_call_contains_chain_and_batch() {
        let chain_id = AlloyU256::from(270u64);
        let batch = AlloyU256::from(42u64);
        let encoded = encode_interop_roots_call(chain_id, batch);
        // 4-byte selector + 32-byte chainId + 32-byte batchNumber
