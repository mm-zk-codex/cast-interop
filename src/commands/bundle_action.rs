...
mod tests {
    ...
    fn wrap_in_rpc_error(hex: &str) -> String {
        // Simulates the format alloy uses for revert errors
        format!("rpc error: code=-32000 data=\"{hex}\"")
    }

    #[test]
    fn decode_revert_reason_standard_error_string() {
        // Build Error("test revert") with selector 0x08c379a0
        let mut data = vec![0x08u8, 0xc3, 0x79, 0xa0];
        let abi_str: Vec<u8> = ("test revert".to_string(),).abi_encode();
        data.extend_from_slice(&abi_str);
        let msg = wrap_in_rpc_error(&format!("0x{}", hex::encode(&data)));
        assert_eq!(
            decode_revert_reason(msg),
            Some("test revert".to_string())
        );
    }

    #[test]
    fn decode_revert_reason_empty_string_revert() {
        let mut data = vec![0x08u8, 0xc3, 0x79, 0xa0];
        let abi_str: Vec<u8> = (String::new(),).abi_encode();
        data.extend_from_slice(&abi_str);
        let msg = wrap_in_rpc_error(&format!("0x{}", hex::encode(&data)));
        assert_eq!(decode_revert_reason(msg), Some(String::new()));
    }

    #[test]
    fn decode_revert_reason_panic_code_1() {
        // Panic(1) with selector 0x4e487b71
        let mut data = vec![0x4eu8, 0x48, 0x7b, 0x71];
        ...
        assert_eq!(decode_revert_reason(msg), Some("panic(1)".to_string()));
    }

    #[test]
    fn decode_revert_reason_panic_code_17_overflow() {
        // Panic(17) = arithmetic overflow
        let mut data = vec![0x4eu8, 0x48, 0x7b, 0x71];
        data.extend_from_slice(&alloy_primitives::U256::from(17u64).to_be_bytes::<32>());
        let msg = format!("error 0x{}", hex::encode(&data));
        assert_eq!(decode_revert_reason(msg), Some("panic(17)".to_string()));
    }

    #[test]
    fn decode_revert_reason_known_custom_error_selector() {
        // BundleAlreadyProcessed selector: 5bba5111 (from code comment)
        // Append a dummy bytes32 argument
        let selector = hex::decode("5bba5111").unwrap();
        let mut data = selector;
        data.extend_from_slice(&[0u8; 32]); // dummy bundleHash
        let msg = format!("rpc 0x{}", hex::encode(&data));
        assert_eq!(
            decode_revert_reason(msg),
            Some("revert: BundleAlreadyProcessed".to_string())
        );
    }

    #[test]
    fn decode_revert_reason_message_not_included_selector() {
        // MessageNotIncluded selector: 32c2e156 (from code comment)
        let data = hex::decode("32c2e156").unwrap();
        let msg = format!("rpc 0x{}", hex::encode(&data));
        assert_eq!(
            decode_revert_reason(msg),
            Some("revert: MessageNotIncluded".to_string())
        );
    }

    #[test]
    fn decode_revert_reason_no_hex_returns_none() {
        assert!(decode_revert_reason("some error without hex".to_string()).is_none());
    }

    #[test]
    fn decode_revert_reason_data_too_short_returns_none() {
        // 2 bytes → less than 4, can't extract selector
        let msg = "0xaabb".to_string();
        assert!(decode_revert_reason(msg).is_none());
    }

    #[test]
    fn decode_revert_reason_unknown_selector_returns_none() {
        // 4 bytes that don't match any known selector
        let data = [0xDEu8, 0xAD, 0xBE, 0xEF, 0x00, 0x00, 0x00, 0x00];
        let msg = format!("0x{}", hex::encode(&data));
        assert!(decode_revert_reason(msg).is_none());
    }

    #[test]
    fn decode_revert_reason_hex_truncated_at_quote() {
        // The closing quote stops hex parsing, so partial data after quote is ignored
        let selector = hex::decode("32c2e156").unwrap();
        let msg = format!("0x{}\"extra garbage", hex::encode(&selector));
        assert_eq!(
            decode_revert_reason(msg),
            Some("revert: MessageNotIncluded".to_string())
        );
